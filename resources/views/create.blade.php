<x-app-layout>
    @if(session()->has('success'))
        ...
        <div class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md" role="alert">
            <div class="flex">
                ...
                <div>
                    <p class="font-bold"> {{ session()->get('success') }} Created successfully</p>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2>Create The Workload for a Member</h2>
                    ...
                    <select id="sep_member" name="sep_member" class="focus:shadow-primary-outline dark:bg-blue-600 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-blue-600 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none">
                        <option disabled selected value class="font-bold"> -- Select Member -- </option>
                        @foreach($sep_data as $sep_data)
                            <option value="{{ $sep_data->id }}">{{ $sep_data->name }}</option>
                        @endforeach
                    </select>
                    <form class="w-full max-w-full px-3 shrink-0  md:flex-0 hidden" id="add-form" action="{{ url('/storeMemberData')}}" method="Post"  enctype="multipart/form-data">
                        @csrf
                        @method('POST')
    ...
    text-align: center;
"></p>
{{--                        Year : <input type="number" name="year" id="year" class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 " required disabled value="{{ now()->year }}">--}}
                        <div id="div_form">
                        <table>
                    ...
                    <table>
                       ...
                       <div class="flex justify-end mt-4">
                        ...
                        <input name="data" id="data" value="" class="hidden">
                        <br>

                        <button type="submit" id="submit"
                                class="text-red-500 border border-red-500 hover:bg-red-500 hover:text-white active:bg-red-600 font-bold uppercase px-8 py-3 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 float-right">
                            Submit
                        </button>
                        </div>
    ...
    <script>
        $( document ).ready(function() {
            $('input').not('#year').addClass('text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block');
        });
